...
def api_register_patient(request):
    try:
        ...
        # Check if the user is valid and has the doctor role
        ...
        if not all([name, username, email, mrno]):
            return Response({
                "status": "error",
                "message": "All fields (name, username, email, mrno) are required."
        ...
        new_user = CustomUser(
            ...
            game3=1,
            status=user_status,
            created_by=user.id,  # 👈 Correct usage
        )
